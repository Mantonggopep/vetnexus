...
const Clients: React.FC<ClientsProps> = ({ owners, pets, sales, communications = [], currency, onAddClient, onAddPatient, onRefresh }) => {
  ...
  const handleSaveClient = async (e: React.FormEvent) => {
    ...
    try {
        const { data } = await api.get('/owners/check-duplicate', {
            params: { name: newClient.name, phone: newClient.phone }
        });
            ...
            <div className="flex-1 grid grid-cols-12 gap-8 overflow-hidden">
                ...
                <div className="col-span-12 md:col-span-8 lg:col-span-9 flex flex-col h-full bg-white/60 backdrop-blur-xl rounded-[2.5rem] border border-white/60 shadow-2xl shadow-slate-200/50 overflow-hidden">
                    <div className="px-8 py-5 border-b border-rose-100/50 flex justify-between items-center bg-white/40 sticky top-0 z-20 backdrop-blur-md">
                        <div className="flex space-x-1 bg-slate-100/50 p-1.5 rounded-2xl overflow-x-auto">
                             {(['patients', 'financials', 'communication', 'login'] as const).map((tab) => (
                                 <button
                                    key={tab}
                                    ...
                                    `}
                                 >
                                    {tab === 'login' ? 'User Login' : tab.charAt(0).toUpperCase() + tab.slice(1)}
                                 </button>
                             ))}
                        </div>
                        {activeTab === 'patients' && (
                             <button 
                                ...
                                className="bg-slate-900 hover:bg-slate-800 text-white px-6 py-3 rounded-2xl text-sm font-bold flex items-center transition-all shadow-xl shadow-slate-900/20 hover:scale-105 active:scale-95 ml-4"
                            >
                                ...
                                {clientPets.map((pet, idx) => (
                                    ...
                                    >
                                        ...
                                        <div className="grid grid-cols-3 gap-2 mt-4 relative z-10">
                                            <div className="bg-slate-50 p-2 rounded-xl text-center"><span className="block text-xs text-slate-400 font-bold uppercase">Age</span><span className="text-sm font-bold text-slate-700">{pet.age}y</span></div>
                                            <div className="bg-slate-50 p-2 rounded-xl text-center"><span className="block text-xs text-slate-400 font-bold uppercase">Sex</span><span className="text-sm font-bold text-slate-700">{pet.gender}</span></div>
                                            <div className="bg-slate-50 p-2 rounded-xl text-center"><span className="block text-xs text-slate-400 font-bold uppercase">Type</span><span className="text-sm font-bold text-slate-700">{pet.species}</span></div>
                                        </div>
                                    </div>
                                ))}
                                {clientPets.length === 0 && (
                                        ...
                                        {clientSales.map(sale => (
                                            <tr key={sale.id} className="hover:bg-rose-50/30 transition-colors">
                                                <td className="px-8 py-5 text-sm font-medium text-slate-600">
                                                    <div className="flex items-center"><Calendar className="w-4 h-4 mr-2 text-rose-400" />{new Date(sale.date).toLocaleDateString()}</div>
                                                </td>
                                                ...
                                                <td className="px-8 py-5"><span className={`px-3 py-1 rounded-full text-xs font-bold ${sale.status === 'Paid' ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-700'}`}>{sale.status}</span></td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                                {clientSales.length === 0 && <div className="p-12 text-center text-slate-400">No financial history available.</div>}
                             </div>
                        )}
                             ...
                             <div className="space-y-6 max-w-3xl mx-auto animate-fade-in-up">
                                {clientComms.map((comm, idx) => (
                                    <div key={comm.id} className="relative flex space-x-6">
                                        {idx !== clientComms.length - 1 && <div className="absolute top-10 left-5 w-0.5 h-full bg-slate-100 -z-10"></div>}
                                        ...
                                        <div className="flex-1 bg-white p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all">
                                            <div className="flex justify-between items-start mb-2"><span className="text-xs font-bold uppercase tracking-wider text-slate-400">{comm.type}</span><span className="text-xs text-slate-400 font-mono">{comm.date}</span></div>
                                            <p className="text-slate-700 font-medium leading-relaxed">{comm.summary}</p>
                                            <div className="text-xs text-slate-400 mt-3 flex items-center"><div className="w-1.5 h-1.5 rounded-full bg-rose-400 mr-2"></div>Logged by {comm.staffName}</div>
                                        </div>
                                    </div>
                                ))}
                                {clientComms.length === 0 && <div className="text-center py-10 text-slate-400"><MessageSquare className="w-10 h-10 mx-auto mb-3 opacity-20" /><p>No communication logs recorded.</p></div>}
                            </div>
                        )}
                                ...
                                <div className="bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm">
                                    <div className="flex items-center gap-4 mb-6">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-slate-500">Manage how this client accesses their portal.</p>
                                        </div>
                                    </div>

                                    <div className="space-y-6">
                                        <div>
                                            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 ml-1">Registered Email (Username)</label>
                                            <div className="flex gap-2">
                                                <input 
                                                    type="email" 
                                                    value={selectedClient.email || ''} 
                                                    readOnly 
                                                    className="flex-1 px-4 py-3 bg-slate-50 border-none rounded-xl text-sm text-slate-600 cursor-not-allowed"
                                            ...
                                            <p className="text-[10px] text-slate-400 mt-2 ml-1">To change email, please edit the main client profile contact info.</p>
                                        </div>

                                        <div className="pt-4 border-t border-slate-100">
                                            <h4 className="font-bold text-slate-800 mb-4 flex items-center"><Lock className="w-4 h-4 mr-2"/> Password Management</h4>
                                            
                                            <div className="flex flex-col gap-3">
                                                <button 
                                                    className="w-full py-3 bg-slate-900 text-white font-bold rounded-xl shadow-lg shadow-slate-900/10 hover:bg-slate-800 transition-all flex items-center justify-center"
                                                    onClick={() => alert(`Reset link sent to ${selectedClient.email || 'client'}`)}
                                                >
                                                    <Mail className="w-4 h-4 mr-2" /> Send Password Reset Email
                                                </button>
                                                <p className="text-center text-xs text-slate-400">
                                                    This will email a secure link for the client to set their own password.
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div className="col-span-2 flex items-center border-l border-dashed border-slate-100 pl-4 h-full">
                            <div className="flex -space-x-3 pl-1">
                                {ownerPets.slice(0, 3).map(pet => (
                                    <img key={pet.id} className="w-9 h-9 rounded-full border-[3px] border-white shadow-sm object-cover bg-slate-100 transition-transform group-hover:translate-x-1" src={pet.imageUrl || `https://ui-avatars.com/api/?name=${pet.name}&background=random`} alt={pet.name} />
                                ))}
                                {ownerPets.length > 3 && <div className="w-9 h-9 rounded-full border-[3px] border-white bg-slate-100 flex items-center justify-center text-[10px] font-bold text-slate-500 shadow-sm z-10">+{ownerPets.length - 3}</div>}
                                {ownerPets.length === 0 && <span className="text-xs text-slate-400 italic pl-2">No pets</span>}
                            </div>
                        </div>
        ...
        <Modal open={isAddClientModalOpen} onClose={() => setIsAddClientModalOpen(false)} title="Register New Client">
             <form onSubmit={handleSaveClient} className="space-y-5">
                 ...
                 <div>
                    ...
                    <textarea className="w-full p-3 bg-slate-50 border-none rounded-xl text-sm focus:ring-2 focus:ring-rose-400 focus:bg-white transition-all resize-none shadow-inner" rows={3} value={newClient.address} onChange={e => setNewClient({...newClient, address: e.target.value})} placeholder="Street address, City, State..." />
                 </div>
                 <div className="pt-2">
                    <button type="submit" className="w-full bg-slate-900 hover:bg-slate-800 text-white py-3.5 rounded-xl font-bold text-sm shadow-xl shadow-slate-900/20 transition-all hover:scale-[1.02] active:scale-95">Create Client Profile</button>
                 </div>
             </form>
        </Modal>
    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-[2rem] shadow-2xl w-full max-w-md overflow-hidden animate-scale-up border border-white/50">
                <div className="px-8 py-5 border-b border-slate-100 flex justify-between items-center bg-white/80 backdrop-blur-md sticky top-0 z-10">
                    ...
                    <button onClick={onClose} className="p-2 bg-slate-100 rounded-full text-slate-500 hover:bg-slate-200 hover:text-slate-800 transition-colors"><X className="w-4 h-4" /></button>
                </div>
...
const Select = ({ label, children, ...props }: React.SelectHTMLAttributes<HTMLSelectElement> & { label: string }) => (
    <div>
        ...
        <div className="relative">
            <select className="w-full px-4 py-3 bg-slate-50 border-none rounded-xl text-sm focus:ring-2 focus:ring-rose-400 focus:bg-white transition-all shadow-sm appearance-none" {...props}>{children}</select>
            <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-slate-400"><ChevronRight className="w-4 h-4 rotate-90" /></div>
        </div>
    </div>
);
